...
conditional_mothers_age_YS <- function(y, s1, b1, U_mat, F_mat, Q){
...
unconditional_mothers_age_YS  <- function(y, s1, U_mat, F_mat, Q){
  n <- nrow(U_mat)
  ...
  actual_ages <- actual_ages - 1 ## because f_mat[1,1] corresponds to age 0 not 1
  dd <- 0
  full_df <- data.frame()
  for(age_mother_focal in actual_ages){
    age_mother_ys <- age_mother_focal - s1 + y
    prob_mother_age_Focal <- probable_ages_of_mothering[(1+age_mother_focal)] ## because f_mat[1,1] corresponds to age 0 not 1
    dd <- dd + prob_mother_age_Focal
    if(age_mother_ys %in% actual_ages){
      temp_df <- data.frame(number = seq(0, Q-1),
                            prob = conditional_mothers_age_YS(y, s1, age_mother_focal, U_mat, F_mat, Q),
...
#'
Matrix_func_age_YS  <- function(y, s1, U_mat, F_mat, Q){
  n <- nrow(U_mat)
  if(y <= s1){stop("Younger sister has to be strictly younger")}
  probable_ages_of_mothering <- unlist(lapply(1:ncol(F_mat), function(x){ mothers_age(U_mat, F_mat, x)}))
  actual_ages <- which(probable_ages_of_mothering != 0)
  ...
  for(mothers_age_Focal in actual_ages){
    ...
    if(mothers_age_YS %in% actual_ages){
      vec <- vec + pm*conditional_mothers_age_YS(y, s1, mothers_age_Focal, U_mat, F_mat, Q)
    }
