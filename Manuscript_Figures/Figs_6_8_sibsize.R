...
for(i in actual_ages){
  for(foc in 1:20){
  for(j in 0:(foc-1)){
    pdf <- ys_PMF_conditional(foc, j, i, u_mat, f_mat, 7)
...
for(i in actual_ages){
  for(foc in 0:20){
    for(j in (foc+1):(foc+i-1)){
      pdf <- os_PMF_conditional(foc, j, i, u_mat, f_mat, 7)
      temp_df <- data.frame(mo_age = i,
                            prob_OS = pdf,
                            number = seq(0,6),
                            age_OS = j,
...
full_sis_df_prob$age_Foc1 <- paste0("Focal aged ", full_sis_df_prob$age_foc_num)

full_sis_df_prob$age_Foc2 = factor(full_sis_df_prob$age_Foc1,levels=full_sis_df_prob$age_Foc1%>%unique())
...
prob_plot <- full_sis_df_prob %>%
  ...
  geom_line(aes(group = factor(number))) + facet_grid(age_Foc2~kin) +
  theme_bw() +
...
prob_plot
ggsave(paste0(fig_out,"/conditioning_prob_sisters_REV.png"), prob_plot, width = 9, height = 10)
...
legend_title_2 <- "Probability Focal has no sisters"
...
all_sis_heat_plot <- all_sis_heat %>%
  ...
  theme(legend.position = "top",
        legend.text = element_text(vjust = 0.75, angle = 45, size = 10),
  ...
  theme(axis.text.x = element_text(size = 12),
        ...
        legend.title = element_text(size=14)) + theme(strip.text = element_text(size = 14))
all_sis_heat_plot
ggsave(paste0(fig_out,"/Heat_plot_age_mom_REV.png"), all_sis_heat_plot, width = 9, height = 5)
