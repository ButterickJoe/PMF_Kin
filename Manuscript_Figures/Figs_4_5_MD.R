...
sim_df_DAU <- readRDS("data/DAU_sim_py3.Rds")
...
joe_dau_50$X3 <- joe_dau_50$number^3*joe_dau_50$prob
...
joe_dau_20$X3 <- joe_dau_20$number^3*joe_dau_20$prob
...
joe_dau_20_means %>% ggplot(aes(x = age, y = mean)) + geom_point() + geom_errorbar(aes(ymin = mean-var, ymax = mean+var))
...
joe_mum_50$X3 <- joe_mum_50$number^3*joe_mum_50$prob
...
joe_mum_50_means %>% ggplot(aes(x = age, y = mean)) + geom_point() + geom_errorbar(aes(ymin = mean-var, ymax = mean+var))
## 20
joe_mum_20 <- list()
for(i in 0:100){
...
joe_mum_20$X3 <- joe_mum_20$number^3*joe_mum_20$prob
...
joe_mum_20_means %>% ggplot(aes(x = age, y = mean)) + geom_point(size = 1, color = "orange") +
  geom_vline(xintercept = 40, linetype = "dashed") + theme_bw() +
  geom_errorbar(aes(ymin = ifelse(mean-var<0,0,mean-var), ymax = mean+var), color = "orange", size = 0.25) +
  ylab("Expectation and variance") + xlab("Age of kin") +
...
md_age_plot <- rbind(dat_50, dat_20) %>% dplyr::filter(mean!=0) %>%
  ggplot(aes(x = age, y = mean, color = age_Focal)) +
  facet_wrap(~kin, scales = "free_x") + geom_point(size = 1) + theme_bw() +
  geom_line(aes(y = var^0.5),linetype="dashed", size = 0.75) +
  geom_line(data = rbind(dat_50, dat_20) %>% dplyr::filter(skew < 50), aes(y = skew/scale), linetype = "solid", size =0.75) +
...
ggsave(paste0(fig_out,"/Age_dists_mum_dau_REV.png"), md_age_plot, width = 9, height = 5)


##### accumulated by age 50 compared to microsimulation
...
m_d_sm_pdf <- rbind(daughter_df_20_50,mother_df_20_50)
m_d_sm_pdf$method1 <- factor(m_d_sm_pdf$method, levels = c("PMF model"))
err_dat <- m_d_sm_pdf %>% filter(number == 0)
err_dat
err_dat$prob <- 0.02

m_d_sm_pdf$age_Focal <- factor(m_d_sm_pdf$age_Focal, levels = c("At 20 years", "At 50 years"))

md_pdf_plot <- m_d_sm_pdf %>% filter(method == "PMF model") %>%
...
m_d_sm_pdf %>% group_by(kin, age_Focal, method) %>%
  summarise(s2 = sum(X2),
            X3 = prob*number^3,
            s3 = sum(X3),
...
test_add <- m_d_sm_pdf %>% group_by(kin, age_Focal, method) %>%
  summarise(s2 = sum(X2),
            X3 = prob*number^3,
            s3 = sum(X3),
            m3 = s3 + 2*expectation^3 - 3*expectation*s2,
            ...
            sd = variance^(0.5),
            prob = prob) %>%
  ungroup()  %>% filter(method == "PMF model") %>%
  filter(age_Focal %in% c("At 50 years", "At 20 years")) %>%
  dplyr::select(-method) %>% mutate(xmap = ifelse(kin == "Mother" , -.5, 4.5))

md_pdf_plot <- m_d_sm_pdf %>% group_by(kin, age_Focal, method) %>%
  summarise(s2 = sum(X2),
            X3 = prob*number^3,
            s3 = sum(X3),
            m3 = s3 + 2*expectation^3 - 3*expectation*s2,
  ...
  theme(strip.text = element_text(size = 14)) +
  theme(legend.position = "none") + scale_x_continuous(breaks = c(0,1,2,3,4,5,6))+
  geom_text(data = test_add,
    aes(x = xmap ,y = 1, label = paste0("mean", "==", round(expectation,2))
    ),parse = TRUE, hjust = 0, vjust = 1, size = 5 ) +
  geom_text(data = test_add,
    aes(x = xmap, y = 0.9, label = paste0("sd", "==", round(sd,2))
    ),parse = TRUE, hjust = 0, vjust = 1 , size = 5) +
  geom_text(data = test_add,
    aes(x = xmap, y = 0.8, label = paste0("skew", "==", round(skew,2))
    ),parse = TRUE, hjust = 0, vjust = 1, size = 5 )
...
md_pdf_plot
ggsave(paste0(fig_out,"/PDF_mum_dau_NEW.png"), md_pdf_plot, width = 9, height = 7)


m_d_sm_pdf %>% dplyr::filter(kin == "Daughters", age_Focal == "At 20 years") %>%
